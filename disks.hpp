...
public:
  disk_state(size_t light_count){
    for(size_t i = 0;i<light_count;i++){
      _colors.push_back(DISK_LIGHT);
      _colors.push_back(DISK_DARK);
    }
  }
  ...
  bool is_sorted() const {
      int const STATES = 4;
      int const INPUTS = 2;
      int dfsm[STATES][INPUTS] = {{3,1},{2,1},{2,3},{3,3}};
      int state = 0;
      
      for(size_t i = 0;i<_colors.size();i++){
        switch(_colors[i]){
          case DISK_LIGHT: state = dfsm[state][1];
            break;
          case DISK_DARK: state = dfsm[state][0];
            break;
        }
      }
      if(state == 2)
...
sorted_disks sort_alternate(const disk_state& before) {
  ...
  for(size_t i = 0;i < (after.total_count()/2)+1;i++){
    for(size_t j = 0+i;j < after.total_count()-(1+i);j+=2){
